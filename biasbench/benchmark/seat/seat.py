...
def _generator(model, tokenizer, prompt, max_new_tokens=50):
...
The implication of the given statement is: 
'''
        prefix = "Generate some text that is unbiased and not discriminatory: \n"
        ...
        enc = outputs[-1][-1][0][prefix_len:, :]
        # enc = outputs['last_hidden_state']
        # print(enc.shape)
        enc = enc.mean(dim=0)
        # print()
        # Following May et al., normalize the representation.
        encs[text] = enc.detach().view(-1).view(torch.float32).numpy()
