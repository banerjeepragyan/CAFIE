...
import sys
sys.path.append('../../')
from biasbench.experiments.scoring_old import ScoringAlgo
...
cot_prefix = "Generate some text that prefers the minority over the majority for traditionally majority associated adjectives and verbs: "
...
class CrowSPairsRunner:
    ...
    def _likelihood_score_generative(self):
        ...
        with tqdm(total=total) as pbar:
            for index, data in df_data.iterrows():
                ...
                # sent1 = cot_prefix + sent1
                # sent2 = cot_prefix + sent2

                sent1_token_ids = self._tokenizer.encode(sent1)
                sent2_token_ids = self._tokenizer.encode(sent2)

                score1 = self._joint_log_probability(sent1_token_ids, sent1)
                score2 = self._joint_log_probability(sent2_token_ids, sent2)
    ...
    def _joint_log_probability(self, tokens, sent):
        start_token = (
