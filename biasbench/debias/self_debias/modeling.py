...
from biasbench.bias_bench.debias.self_debias.generation import (
    SelfDebiasingLogitsProcessor,
    SelfDebiasingGPT2LMHeadModel,
    SelfDebiasingT5ForConditionalGeneration,
    SelfDebiasingAutoModelForCausalLM,
)
...
class AutoLMWrapper(ABC):
    ...
    def get_token_probability_distribution(
        ...
        """
        output_choice_ids = []
        kwargs = {"add_prefix_space": True} if isinstance(self, AutoWrapper) else {}
        for word in output_choices:
            tokens = self._tokenizer.tokenize(word, **kwargs)
            assert (
                len(tokens) == 1
...
class AutoWrapper(AutoLMWrapper):
    def __init__(self, model_name: str = "tiiuae/falcon-7b", use_cuda: bool = True):
        """
        :param model_name: the name of the pretrained GPT2 model (default: "gpt2-xl")
        :param use_cuda: whether to use CUDA
        """
        ...
        # AutoConfig.register('sdb', SelfDebiasingAutoConfig)
        # AutoModelForCausalLM.register(SelfDebiasingAutoConfig, SelfDebiasingAutoModelForCausalLM)

        # self._model = SelfDebiasingAutoModelForCausalLM.from_pretrained('tiiuae/falcon-7b-instruct', trust_remote_code=True)
        # config = SelfDebiasingAutoConfig.from_pretrained(model_name, trust_remote_code=True, device_map='auto')
        # print(config)
        # model = SelfDebiasingAutoModelForCausalLM.from_pretrained(model_name=model_name, pretrained_model_name_or_path=model_name, config=config, trust_remote_code=True)
        # self._model

        self._model = SelfDebiasingAutoModelForCausalLM.from_pretrained(model_name, trust_remote_code=True)

        # config = AutoConfig.from_pretrained(model_name)
        # config['SelfDebiasingAutoModelForCausalLM'] = config['AutoModelForCausalLM']
        # self._model = AutoModelForCausalLM.from_config(config)
        # print(config)
        # config['SelfDebiasingAutoModelForCausalLM']

        # self._model = SelfDebiasingAutoModelForCausalLM.from_pretrained(
        #     model_name,
    ...
    def generate_self_debiasing(
        ...
        **kwargs,
        ...
        #     tokenizer=self._tokenizer,
        # )
        self._model.logits_processor = SelfDebiasingLogitsProcessor(
            num_debiasing_prefixes=len(debiasing_prefixes),
            decay_constant=decay_constant,
            epsilon=epsilon,
            debug=debug,
