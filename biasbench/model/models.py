...
class GPT2LMHeadModel:
    def __new__(self, model_name_or_path):
        return transformers.GPT2LMHeadModel.from_pretrained(model_name_or_path, device_map='auto', output_hidden_states=True)

class T5ForConditionalGeneration:
    def __new__(self, model_name_or_path):
        return transformers.T5ForConditionalGeneration.from_pretrained(model_name_or_path)
...
class _SentenceDebiasModel:
    def __init__(self, model_name_or_path, bias_direction):
        def _hook(module, input_, output, bias_direction):
            ...
            bias_direction = bias_direction.to(output[-1][-1].device)

            # Debias the representation.
            for t in range(output[-1][-1].size(1)):
                output[-1][-1][:, t] = output[-1][-1][:, t] - torch.ger(
                    torch.matmul(output[-1][-1][:, t], bias_direction), bias_direction
...
class SentenceDebiasAutoModelForCausalLM(_SentenceDebiasModel):
    def __new__(self, model_name_or_path, bias_direction):
        super().__init__(self, model_name_or_path, bias_direction)
        model = transformers.AutoModelForCausalLM.from_pretrained(model_name_or_path, output_hidden_states=True)
        # model.transformer.register_forward_hook(self.func)
        model.gpt_neox.register_forward_hook(self.func)
        return model
...
class DropoutAutoModelForCausalLM:
    def __new__(self, model_name_or_path):
...
class SelfDebiasAutoModelForCausalLM:
    def __new__(self, model_name_or_path):
        model = AutoWrapper(model_name_or_path, use_cuda=True)
        return model