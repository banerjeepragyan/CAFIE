    ...
    for i in tqdm(range(n_batches), desc="Encoding gender examples"):
        ...
        with torch.no_grad():
            ...
            embedding_male = model(
                input_ids=male_input_ids, attention_mask=male_attention_mask, output_hidden_states=True
            ).hidden_states[-1]
            ...
            embedding_female = model(
                input_ids=female_input_ids, attention_mask=female_attention_mask, output_hidden_states=True
            ).hidden_states[-1]
        ...
        all_embeddings_male.append(embedding_male.cpu().to(dtype=torch.float32).numpy())
        all_embeddings_female.append(embedding_female.cpu().to(dtype=torch.float32).numpy())
...
def compute_race_subspace(data, model, tokenizer, batch_size=32):
    ...
    for i in tqdm(range(n_batches), desc="Encoding race examples"):
        ...
        with torch.no_grad():
            embedding_r1 = model(
                input_ids=r1_input_ids, attention_mask=r1_attention_mask, output_hidden_states=True
            ).hidden_states[-1]
            # print(embedding_r1)
            ...
            embedding_r2 = model(
                input_ids=r2_input_ids, attention_mask=r2_attention_mask, output_hidden_states=True
            ).hidden_states[-1]
            ...
            embedding_r3 = model(
                input_ids=r3_input_ids, attention_mask=r3_attention_mask, output_hidden_states=True
            ).hidden_states[-1]
        ...
        # x = embedding_r1.detach().cpu().to(dtype=torch.float32)
        # print("==", x)
        all_embeddings_r1.append(embedding_r1.cpu().to(dtype=torch.float32).numpy())
        all_embeddings_r2.append(embedding_r2.cpu().to(dtype=torch.float32).numpy())
        all_embeddings_r3.append(embedding_r3.cpu().to(dtype=torch.float32).numpy())
