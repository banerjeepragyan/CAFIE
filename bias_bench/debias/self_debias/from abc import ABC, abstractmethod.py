...
class AutoWrapper(AutoLMWrapper):
    def __init__(self, model_name: str = "tiiuae/falcon-7b", use_cuda: bool = True):
        ...
        # print(config)
        try:
            config.auto_map['SelfDebiasingAutoModelForCausalLM'] = config.auto_map['AutoModelForCausalLM']
        except:
            pass
        self._model = SelfDebiasingAutoModelForCausalLM.from_config(config, trust_remote_code=True)     
