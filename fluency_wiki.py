...
for i in tqdm(range(0, encodings.input_ids.size(1), 200)):
    begin_loc = max(i + 200 - 200, 0)
    end_loc = min(i + 200, encodings.input_ids.size(1))
    trg_len = end_loc - i  # may be different from stride on last loop
    input_ids = encodings.input_ids[:, begin_loc:end_loc].to('cuda')
    prompt = tokenizer.decode(input_ids.tolist()[0])
    # print(prompt)
    runner = ScoringAlgo(
        mdl=model,
        model_name_path='g',
