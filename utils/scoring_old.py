...
class ScoringAlgo:
    ...
    def calculate_new_pdf (self, probs_origin, scores, scres2, avg, thres, sf, lmbd, iil):
        ...
        scres2[:, :, iil] = 0
        d1 = torch.exp(torch.norm(scores))
        d2 = torch.exp(torch.norm(scres2))
    ...
    def generate_sentences (self, prompt, sent_len, male_words, female_words, new_cnt_words, male_words_2, female_words_2, context, bt):
        ...
        for i in range(sent_len):
            ...
            outputs_origin_trimmed = torch.index_select(outputs_origin.to(device), 1, torch.Tensor(reqd_indices_origin).long().to(device))
            outputs_countr_trimmed = torch.index_select(outputs_countr.to(device), 1, torch.Tensor(reqd_indices_countr).long().to(device))
            ...
            ids_origin_trimmed = torch.index_select(input_ids_origin.to(device), -1, torch.Tensor(reqd_indices_origin).long().to(device)).to(device)
